...
log2FC <- log2((cpmBPTFko+1)/(cpmWt+1))
log2avrCPM <- log2(rowMeans(cpm) + 1)
...
## Plot
Heatmap(log2cpmFC, name = "log2FC w.r.t. mean", show_row_names = FALSE)

```

We can appreciate a strong aggrement between replicates within a condition.


## Visualize peaks on genome tracks

Let's choose a peak that is showing strong differential binding between BPTFko and wt, and visualize the 
