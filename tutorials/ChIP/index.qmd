...
Let's calculate the enrichment values for sample `wt_rep1`.
...
enr <- log2cntNorm[, "ChIPseq_ESC_CTCF_IP_wt_rep1"] - log2cntNorm[, "ChIPseq_ESC_CTCF_input_wt_rep1"]
enrTiles <- enr > 2
...
::: {.callout-tip}
## Exercise
Choose another sample and estimate the average fragment size. How does it compare to `ChIPseq_ESC_CTCF_IP_wt_rep1`? By how much should the reads be shifted on average to get the TF binding location, and by what direction?
:::
...
peak calling per sample and comparing across replicates
