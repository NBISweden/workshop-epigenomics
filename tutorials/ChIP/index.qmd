...
```{r}
library(rtracklayer)
wigfnames <- c(tempfile(fileext=".wig.gz"),tempfile(fileext=".wig.gz"))
qExportWig(proj, file=wigfnames, binsize=100L, scaling=TRUE, collapseBySample=TRUE)
gr1 <- import(wigfnames[1])
gr2 <- import(wigfnames[2])
annot <- import(file.path(td,"extdata/hg19sub_annotation.gtf"))
colnames(mcols(annot))[c(2,5,8)] <- c("feature","group","id")

library(Gviz)
axisTrack <- GenomeAxisTrack()
dTrack1 <- DataTrack(range=gr1, name="Sample 1", type="h")
dTrack2 <- DataTrack(range=gr2, name="Sample 2", type="h")
aTrack <- AnnotationTrack(annot, name="Genes")
plotTracks(list(axisTrack, dTrack1, dTrack2, aTrack), chromosome="chr3", extend.left=1000)
