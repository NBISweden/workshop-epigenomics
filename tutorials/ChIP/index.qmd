...
Note that we are looking at pairwise overlaps. Let's create a common set of peaks that we can use to create a count matrix across all samples so we can have a more global and quantitative overview of how well the samples agree.
...
rd <- as.data.frame(peaks) %>% 
  rename(seqnames = "chromosome", start = "chrStart", end = "chrEnd", 
         width = "peakWidth", strand = "chrStarnd")
rd <- DataFrame(rd)
...
profile <- qProfile(proj, query = peaks, upstream = 1000, 
                    downstream = 1000, orientation = "any", 
                    shift = 75, clObj = cl)
